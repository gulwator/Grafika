...
void bounce_ball(Pong* pong)
{
    ...
    if ((pong->ball.x - pong->ball.radius < 50) && ((pong->ball.y - pong->ball.radius >= pong->left_pad.y) && (pong->ball.y - pong->ball.radius <= pong->left_pad.y + pong->left_pad.height))) {
        // indulási pont
        pong->ball.x = pong->ball.radius + 50;
        //sebesség
        pong->ball.speed_x *= -1;
